  ...
  return (
    <Combobox value={selectedProjectIds} onChange={changeSelected} multiple>
      <div className="relative mt-1">
        ...
        <Combobox.Options className="absolute z-30 mt-1 max-h-96 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
          {filteredProjects.length === 0 ? (
            <div className="relative cursor-default select-none py-2 px-4 text-gray-700">
              No projects found{!query.length && ` - check your API token`}
            </div>
          ) : (
